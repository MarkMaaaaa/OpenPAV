...
from dataPreprocessing_M1_global import dataSelection_simplified, trajectory_predict_M1_global
...
def objective_func_simplified(para_list, df_selected_list = df_selected_list_):
    scales = np.array([1, 1, 10, 1, 1]) 
    para_list = para_list * scales
    error = 0
    for df_selected_ in df_selected_list:
        df_results = trajectory_predict_M1_global(para_list, df_selected_)
...
# problem_dimension = 5
# upper_bound = [3,     4,   3,     3,   3]
# lower_bound = [1,     1,   2.5,   0,   0]
# bo = BO(problem_dimension, upper_bound, lower_bound, init_sample_num = 30)
# bo.minimization(objective_func_simplified)

problem_dimension = 5
ordinal_upper_bound = [3,    4,   3,   3,   3]
ordinal_lower_bound = [1,    1,   2.5,   0,   0]
upper_bound = [2.7,  3.3,  3,   1,   3]
lower_bound = [2.7,  3.3,  2.5,   1,   0]
init_sample_num = 5
